...
int main(void) {
  ...
  struct set * r1 = set_intersect(set1, set2);
  struct set * r2 = set_intersect(set2, set3);
  struct set * r3 = set_intersect(set1, set3);

  struct set * empty = empty_set();

  assert(set_equal(r1, empty));
  assert(set_equal(r2, empty));
  assert(set_equal(r3, empty));
